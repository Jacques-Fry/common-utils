...
public class ImageUtil {
     ...
     * @param alpha         透明度：alpha 必须是范围 [0.0, 1.0] 之内（包含边界值）的一个浮点数字
     */
    public static void pressText(String pressText, InputStream inputStream, String destImageFile, String fontName, int fontStyle, Color color, int fontSize, int x, int y, float alpha) {
        try {
            ...
            // 文本长度
            int textLength = getLength(pressText);

            // 验证高度
            if (image.getHeight() - fontSize < y * 2) {
                y = (image.getHeight() - fontSize - 2) / 2;
            }
            if (image.getHeight() - fontSize < -(y * 2)) {
                y = -((image.getHeight() - fontSize - 2) / 2);
            }
            // 验证宽度
            if (image.getWidth() - textLength * fontSize < x * 2) {
                x = (image.getWidth() - textLength * fontSize - 2) / 2;
            }
            if (image.getHeight() - textLength * fontSize < -(x * 2)) {
                x = -((image.getHeight() - textLength * fontSize - 2) / 2);
            }
            ...
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
                    alpha));
            // 在指定坐标绘制水印文字
            g.drawString(pressText, (width - (textLength * fontSize))
                    / 2 + x, (height - fontSize) / 2 + y);
            g.dispose();
     ...
     * @param alpha         透明度：alpha 必须是范围 [0.0, 1.0] 之内（包含边界值）的一个浮点数字
     */
    public static void pressImage(String pressImg, InputStream srcImageFile, String destImageFile,
                                  int x, int y, float alpha) {
        try {
            ...
            // 验证高度
            if (image.getHeight() - heightBiao < y * 2) {
                y = (image.getHeight() - heightBiao - 10) / 2;
            } else if (image.getHeight() - heightBiao < -(y * 2)) {
                y = -((image.getHeight() - heightBiao - 10) / 2);
            }
            // 验证宽度
            if (image.getWidth() - widethBiao < x * 2) {
                x = (image.getWidth() - widethBiao - 20) / 2;
            }
            if (image.getWidth() - widethBiao < -(x * 2)) {
                x = -((image.getWidth() - widethBiao - 20) / 2);
            }
            ...
            // 绘制水印
            g.drawImage(srcBiao, (width - widethBiao) / 2 + x,
                    (height - heightBiao) / 2 + y, widethBiao, heightBiao, null);
            // 水印文件结束
            g.dispose();
            ImageIO.write(image, "JPEG", new File(destImageFile));
        } catch (Exception e) {
...
//    public static void main(String[] args) {
    // 1-缩放图像：
    // 方法一：按比例缩放
//        ImageUtil.scale("D:\\test\\test\\headpic.jpg", "D:\\test\\test\\headpicX2.jpg", 2, true);//测试OK
    // 方法二：按高度和宽度缩放
...
//
//        // 6-给图片添加图片水印：
//        ImageUtil.pressImage("D:/test/test/a.png", "D:/test/test/headpic.jpg","D:/test/test/headpic_suiying.jpg", 0, 0, 0.1f);//测试OK
//    }
