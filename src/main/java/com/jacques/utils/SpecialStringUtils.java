...
public class SpecialStringUtils {
    public final static String UNICODE_REG =
            // ：CJK
            // 统一表意符号
            // (CJK
            // Unified
     ...
     * @param specialStr 传入的特殊字符
     * @param changeStr 需要变更的字符
     * @return java.lang.String
     */
    public static String specialStrChange(String specialStr,String changeStr) {
        try {
            Pattern pattern = Pattern.compile(UNICODE_REG);
            StringBuilder sbBuffer = new StringBuilder();
            for (int i = 0; i < specialStr.length(); i++) {
                char c = specialStr.charAt(i);
                String temp = String.valueOf(c);
                Matcher matcher = pattern.matcher(temp);
                if (matcher.find()) {
                    sbBuffer.append(temp);
                } else {
                    sbBuffer.append(changeStr);
                }
            }
